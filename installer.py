"""
Installer - Extractor de Trades v2.1.0
...
STEPS = [
    ...
    ("PIL",        "Instalando Pillow (imágenes)..."),
    ("winshell",   "Instalando winshell..."),
    ("chromium",   "Instalando Chromium..."),
    ("shortcut",   "Creando acceso directo..."),
]
...
class InstallerApp:
        ...
        for i, (pkg, label) in enumerate(STEPS):
            ...
            if pkg == "pip":
                ...
                if result.returncode == 0:
                    status = "skip"
                    self._log("pip ya está disponible", FG2)
                else:
            ...
            # ── Chromium ─────────────────────────────────────
                ...
                if "ok" in chromium_check.stdout:
                    status = "skip"
                    self._log("Chromium ya está instalado", FG2)
                else:
            ...
            # ── Acceso directo ────────────────────────────────
                ...
                lnk_path = os.path.join(app_dir, "Extractor Trades.lnk")
                if os.path.exists(lnk_path):
                    status = "skip"
                    self._log("Acceso directo ya existe", FG2)
                else:
                    script = os.path.join(app_dir, "crear_acceso_directo.py")
                    ok = run_cmd([sys.executable, script])
                    status = "ok" if ok else "skip"
                    self._log("Acceso directo creado" if ok
                              else "No se pudo crear el acceso directo (opcional)",
                              OK_COLOR if ok else WARN_COL)

            # ── Python packages ──────────────────────────────
            else:
                check_pkg = {"PIL": "PIL", "winshell": "winshell", "playwright": "playwright"}.get(pkg, pkg)
                if is_installed(check_pkg):
                    status = "skip"
    ...
    def _launch_app(self):
        self.success = True
        self.root.destroy()
        # Only launch app if running standalone (double click on installer.py directly)
        # When called from Iniciar.vbs, the vbs handles launching extractor_ui.py
        if not getattr(sys, "frozen", False) and len(sys.argv) <= 1:
            # Check if called directly (not from vbs which passes no special args)
            # Use an env variable set by vbs to know if we should launch
            if not os.environ.get("EXTRACTOR_VBS_LAUNCH"):
